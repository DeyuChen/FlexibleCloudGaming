...
int main(int argc, char *argv[]){
    ...
    string fname = client_dir + "PSNR_diff.txt";
    ofstream ofs(fname);
    assert(ofs.is_open());
    ...
    short *diff0 = new short[3 * WIDTH * HEIGHT];
    unsigned char *diff1 = new unsigned char[3 * WIDTH * HEIGHT];
    unsigned char *diff2 = new unsigned char[3 * WIDTH * HEIGHT];
    ...
    while (true){
        ...
        */
        ...
        fname = server_dir + to_string(pts) + string(".diff0");
        if (stat(fname.c_str(), &buffer) != 0)
            break;
                
        ifstream ifs(fname);
        assert(ifs.is_open());
        ifs.read((char*)diff0, 3 * WIDTH * HEIGHT * sizeof(short));
        ifs.close();
        
        fname = server_dir + to_string(pts) + string(".diff1");
        ifs.open(fname);
        assert(ifs.is_open());
        ifs.read((char*)diff1, 3 * WIDTH * HEIGHT);
        ifs.close();
        
        fname = server_dir + to_string(pts) + string(".diff2");
        ifs.open(fname);
        assert(ifs.is_open());
        ifs.read((char*)diff2, 3 * WIDTH * HEIGHT);
        ifs.close();
        
        double MSE1 = 0;
        unsigned char vs, vc;
        for (int i = 0; i < 3 * WIDTH * HEIGHT; i++){
            int diff = (int)diff0[i] - (2 * ((int)diff1[i] - 127));
            if (diff > 255 || diff < -255) 
                cout << "error: " << diff << " " << diff0[i] << " " << max(min((int)(2 * (diff1[i] - 127)), 255), 0) << endl;
            MSE1 += diff * diff;
        }
        MSE1 /= 3 * WIDTH * HEIGHT;
        double PSNR1 = 10 * log10(R / MSE1);
        
        double MSE2 = 0;
        for (int i = 0; i < 3 * WIDTH * HEIGHT; i++){
            //int diff = (int)diff1[i] - (int)diff2[i];
            int diff = (int)diff0[i] - (2 * ((int)diff2[i] - 127));
            MSE2 += diff * diff;
        }
        MSE2 /= 3 * WIDTH * HEIGHT;
        double PSNR2 = 10 * log10(R / MSE2);
        
        ofs << pts << " " << MSE1 << " " << PSNR1 << " " << MSE2 << " " << PSNR2 << endl;
        cout << pts << " " << MSE1 << " " << PSNR1 << " " << MSE2 << " " << PSNR2 << endl;
        double rssim, gssim, bssim;
