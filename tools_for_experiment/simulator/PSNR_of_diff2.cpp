...
int main(int argc, char *argv[]){
    ...
    string fname = client_dir + "PSNR_diff2.txt";
    ofstream ofs(fname);
    assert(ofs.is_open());
    ...
    unsigned char *diff3 = new unsigned char[3 * WIDTH * HEIGHT];
    ...
    while (true){
        ...
        */
        ...
        fname = client_dir + to_string(pts) + string(".diff3");
        ifs.open(fname);
        assert(ifs.is_open());
        ifs.read((char*)diff3, 3 * WIDTH * HEIGHT);
        ifs.close();
        
        double MSE = 0;
        unsigned char vs, vc;
        for (int i = 0; i < 3 * WIDTH * HEIGHT; i++){
            int diff = (int)diff2[i] - (int)diff3[i];
            MSE += diff * diff;
        }
        MSE /= 3 * WIDTH * HEIGHT;
        double PSNR = 10 * log10(R / MSE);
