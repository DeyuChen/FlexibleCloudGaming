...
public:
    ...
    vertex(float x1, float y1, float z1, unsigned char r1, unsigned char g1, unsigned char b1) : 
        _myVertex(x1, y1, z1), _vertexNormal(0.0, 0.0, 0.0), 
        _bActive(true), _cost(0), _minCostNeighbor(-1),
        _index(-1), _QTriArea(0)
    {
...
//    operator>>(istream& is, vertex& vi);
    ...
    const unsigned char* getArrayRGB() const {
        return _rgb;
    }
    ...
    const Vec3& getVertNomal(){return _vertexNormal;}
